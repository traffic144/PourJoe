...
\subsection{Equivalent binary trees}
...
\begin{lemma}
Given $G_k\backslash E' \in \mcalg_k$, $(k - \card{E'}) \leq \min_{v\in V} h(v)$
\end{lemma}




%For each graph $G_k\backslash E'$, a road map $\left(G_k\backslash E',E_*\right)$ fulfils $\card{E'} + \card{E_*} = k$.
