...
\end{theorem}
\begin{proof}
Let $G \in \mcald_k$ and $e$ be an edge in $\eright{G}$. It exists an edge $e^*$ in $T_G$ for which $T_{G \backslash \set{e}}$ can be obtained be removing $e^*$ from $T_G$, keeping the tree containing $t$, and applying a new contraction. Let $v$ be the "top" of edge $e^*$, {\em i.e.} node $v$ such that $e^* = \set{u, v}$ and $v \in P(e^*)$. We distinguish two cases :
...
When edge $e$ is removed from $G$, edge $e^*$ and its descendants are withdrawn in the \ebt , so after the contraction, $T_v$ becomes $T_w$, the subtree of root~$w$. All the leaves of $T_w$ have initially a depth greater than $k$ so by removing $e$ from $G$, all the leaves of $T_w$ have a depth greater than $k-1$. All other leaves outside $T_v$, preserve their depth which is greater than $k$. Consequently all leaves of $T_{G \backslash \set{e}}$ have a depth greater than $k-1$.
\end{itemize}
...
\begin{theorem}
For any graphs $G, G' \in \mcald_k$, $c_{\emph{\scriptsize{MS}}}(G) = c_{\emph{\scriptsize{MS}}}(G') = c_k$.
\label{th:equalcompetitive}
\end{theorem}
\begin{proof}
We prove it by induction. If $k = 0$, then for every graph of $\mcald_0$, $\cms(G) = c_0 = 1$ because any path the traveller takes is open.
...
For any $1 \le j \le 2^k$, let $p_j^A$ denote the probability that the traveller visits the $\ith{j}$ node at depth $k$, noted $v_j$ (indexing nodes from left to right in the \ebt ~representation). Note that $\sum_{j=1}^{2^k} p_j^A = 1$.

Let $P_j$ be the set of simple \stpath ~of length 1 which traverses node $v_j$.
Given a set of $E_*$ of blocked edges, we note $J(E_*)$ the set of node $v_j$ at depth $k$ where each paths in $P_j$ is not blocked. For any node $v_j$ of depth $k$, we can construct a set of blocked edges such that $J(E_*) = \set{v_j}$. Indeed, let $e$ be the edge linking the sibling of $v_j$ and $P\left(v_j\right)$. By blocking the set of edges $E_j$ containing $e$,  $U(e)$, $U(U(e))$ ... $U^{k-1}(e)$, we block $1+2+4+\ldots+2^{k-1} = 2^k - 1$ node $v_{j'}$, $j' \neq j$. As we consider only simple paths, each path in $P_j$ are open. It comes that :
\[
c_A(G, E_j) = p_j^A + (2+c_{k-1})\sum_{j' \neq j}p_{j'}^A = p_j^A + (2+c_{k-1})\times(1-p_{j}^A)
\]
For a set of blocked edges $E_*$, if $v_j \in J(E_*)$, then the distance traversed when passing through it is $1$. If $v_j \notin J(E_*)$, the distance traversed is at most $2 + c_{k-1}$,
\[
c_A(G, E_*) \leq \sum_{v_j\in J(E_*)} p_j^A+  (2+c_{k-1})\sum_{v_j\notin J(E_*)}p_j^A
\]

We note $j^*$ the index of the node $v_{j^*}$ in $J\left(E_*\right)$ where $p_{j^*}^A$ is the smallest. As $1 < 2 + c_{k-1}$, the greater $\sum_{v_j\in J(E_*)} p_j^A$ is, the lower $c_A(G, E_*)$ is, therefore
\[
c_A(G, E_*) \le p_{j^*}^A + (2+c_{k-1})\sum_{j\neq j^*}p_j^A = c_A(G, E_{j^*})
\] 
...
The \mts ~with the smallest value $c_{A,\mcalr_k}$ fulfils $p_j^A = \frac{1}{2^k}$ for any $1 \le j \le 2^k$. This value depends on $k$ and $c_{k-1}$, not on the graph itself. For $G,G' \in \mcald_k$, $\cms(G) = \cms(G') = c_k = \min_{A} c_{A,\mcalr_k} = \frac{1}{2^k} + \left(1-\frac{1}{2^k}\right)\left(2+c_{k-1}\right)$.
...
We obtain the iterative formula for $c_k$ by a sum from $j = 1$ to $k$ :
\[
