...
In Section~\ref{sec:roadatlas}, we present sets $\mathcal{R}_k$ (called road atlases) of \textit{road maps}, {\em i.e.} pairs $\left(G,E_*\right)$ with graph $G$ and blocked edges $E_*$. These road maps, together with associated binary trees, are a means to study the performance of memoryless strategies.
...
We focus on the behavior of a traveller traversing road maps $\left(G_k,E_*\right)$ composed of graph $G_k$ and at most $k$ blocked edges with an \mts . Blocked edges in $E_*$ are at the right of axis $\deltavert$. Indeed, blocking edges left to $\deltavert$ affects negligibly the total distance traversed by a traveller. Let us suppose that he traverses graph $G_k$ and has already discovered some blocked edges $E' \subseteq E_*$. He tries now to reach $t$ on graph $G_k\backslash E'$, ignoring that the undiscovered blocked edges are $E_*' = E_* \backslash E'$. From the memoryless point of view, the graph considered at this step is $G_k\backslash E'$, without taking past moves into account. Even if road map $\left(G_k,E_*\right)$ is the instance, an \mts ~might take decisions in function of graphs $G_k\backslash E'$, where $E' \subseteq E_*$.
We note $\mcalg$ the set of all the subgraphs of $G_k$, {\em i.e.} graphs $G_k\backslash E'$ with at most $k$ edges $E'$ right to $\deltavert$, for any integer $k \geq 1$. We call them \textit{diamond graphs} because of their appearance, diamonds joined together. Formally, we set $\mcalg = \bigcup_{k=1}^{+\infty} \set{G_k\backslash E' : \card{E'} \leq k}$. For any graph $G \in \mcalg$, we partition its edges, noted $E_G$, in two sets $\eleft{G}$ (left to axis $\deltavert$) and $\eright{G}$ (right to axis $\deltavert$). Road maps $\left(G,E_*\right)$ composing road atlases $\mcalr_k$ contain graphs in $\mcalg$. 
...
To any diamond graph $G$ of $\mcalg$, we associate a \textit{diamond binary tree} (\ebt ) noted $T_G$ which represents all \textit{branching nodes} from $t$ in graph $G$. The \ebt ~is obtained thanks to a breadth-first search (BFS) from $t$ in direction of nodes on axis $\deltavert$. Node $t$ is the root of the \ebt . Graphically, each node from which there is a \textit{branching node}, {\em i.e.} there are two possible directions for the next move of the BFS, provides two extra edges in the tree (see nodes $t$, $v_1$, $v_2$ and $v_3$ in Figure~\ref{subfig:ebt_2}). We note $T_\varnothing$ the empty tree. Any nonempty tree is a triplet $(v, T_a, T_b)$ with a root $v \in V$ and two subtrees $T_a$ and $T_b$. Figures~\ref{subfig:ebt_1},~\ref{subfig:ebt_2} and~\ref{subfig:ebt_3} illustrate the construction of the \ebt . 
...
To put the definition of \ebt s formally, let $\bfs\left(v\right)$ denote the set of nodes visited just after node $v$ in the BFS starting from $t$. For example, $v_1 \in \bfs\left(t\right)$ and $v_3 \in \bfs\left(v_1\right)$. We force this BFS to stop on axis $\deltavert$ such that $v \in \deltavert \Rightarrow \bfs\left(v\right) = \emptyset$. We define below function $\bintree$ which gives the construction of tree $T_G$ fulfilling $\bintree \left(t\right) = T_G$.
