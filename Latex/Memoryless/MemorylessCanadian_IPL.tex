...
For any graph $G$ of $\mcalg_k$, we define an equivalent binary tree (\ebt ) noted $T_G$ which represents all changes of direction from $t$ in graph $G$. We note $T_\varnothing$ the empty tree. Any nonempty tree is a triplet $(v, T_a, T_b)$ with a root $v \in V$ and two subtrees $T_a$ and $T_b$.
...
\caption{Changes of directions from $t$}
\label{subfig:ebt_2}
\end{subfigure}
\begin{subfigure}[b]{0.3\columnwidth}
\centering
...
\begin{theorem}
For any graph $G_k\backslash E' \in \mcalg_k$, $T_{G_k\backslash E'} \in \compbin{k-1-\card{E'}}$.
\label{th:completebinary}
\end{theorem}
\begin{proof}
If $E' = \emptyset$, the EBT $T_{G_k}$ is a complete binary tree, so for any $0\le j \le k-1$, $\card{E_{T_{G_k}}(j-1)}= 2^j$.
...
\begin{subfigure}[b]{0.3\columnwidth}
\centering
\scalebox{.42}{\input{graphics/proofth1_a}}
\caption{Example of graph $G_3\backslash E' \in \mcalg_3$, with $\card{E'} = 1$. Edge $e$ (red) is about to be removed from this graph.}
\label{subfig:proofth1_a}
\end{subfigure}
~
\begin{subfigure}[b]{0.3\columnwidth}
\centering
\scalebox{.58}{\input{graphics/proofth1_b}}
\caption{The EBT $T_{G_3\backslash E'}$ which belongs to $\compbin{1}$ because $k-1-\card{E'} = 1$. Edges of depth 0 and 1 are framed.}
\label{subfig:proofth1_b}
\end{subfigure}
~
\begin{subfigure}[b]{0.3\columnwidth}
\centering
\scalebox{.58}{\input{graphics/proofth1_c}}
\caption{The EBT $T_{G_3\backslash \left(E' \cup \set{e}\right)}$ which belongs to $\compbin{0}$ because $k-1-\card{E' \cup \set{e}} = 0$. Edges of depth 0 are framed.}
