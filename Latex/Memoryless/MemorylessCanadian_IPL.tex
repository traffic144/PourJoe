...
We specify a \textit{road atlas} $\mathcal{R}$ which is a set of road maps. To do so, we start by introducing a family of graphs $G_k$ and establishing blocked edges on them. The objective is to assess the competitive ratio of randomized \mts es on set $\mathcal{R}$.

\subsection{Graphs $G_i$} \label{subsec:Gi}
...
We focus on road maps $\left(G_i,E_*\right)$ composed of graph $G_i$ and $i$ blocked edges $E_*$ at the right of axis $\deltavert$ in graph $G_i$. Indeed, blocking edges left to $\deltavert$ affects negligibly the total distance traversed by a traveller. Let us suppose that the traveller, guided by a \mts , traverses graph $G_i$ and has already discovered some blocked edges $E' \subseteq E_*$. He tries now to reach $t$ on graph $G_i\backslash E'$, ignoring that the undiscovered blocked edges are $E_*' = E_* \backslash E'$.
We note $\mcalg$ the set of all these possible graphs. Formally,
\[
\mcalg = \bigcup\limits_{i=1}^{+\infty} \mcalg_i \mbox{with}~
\mcalg_i = \set{G_i\backslash E' : \card{E'} \leq i}
\]
...
For any graph $G$ of $\mcalg_k$, we define an equivalent binary tree (\ebt ) noted $T_G$ which represents all \textit{separations} (notion defined below) from $t$ in graph $G$. We note $T_\varnothing$ the empty tree. Any nonempty tree is a triplet $(v, T_a, T_b)$ with a root $v \in V$ and two subtrees $T_a$ and $T_b$.
...
%Commande \compbin{D} pour la famille des arbres binaires complets until depth D
We introduce notations which characterize some elements in an EBT. For any edge $e$ in EBT $T$, we note $P(e)$ the parent edge of $e$. Let $U(e)$ denote the ``uncle'' of edge $e$, which has the same parent than $P(e)$. Put formally, $U(e) = \set{e' \in T : P(e') = P\left(P(e)\right) = P^2(e)}$. For any EBT $T$, we define recursively a set of edges $E_T(k)$ which is the set of edges with depth $k$. Set $E_T(0)$ represents the edges connected to the root $t$ and $E_T(k) = \set{e : P(e) \in E_T(k-1)}$. For a given tree $T$, we note $D_{min}(T)$ the minimum depth of the leaves of $T$. We constitute then the set of graph $\mcald_k = \set{G \in \mcalg : D_{min}(T_G) \geq k}$

We study the competitiveness of \mts es on the road atlas $\mathcal{R}$ composed of road maps with graph of $\mcald_k$ with $k$ blocked edges. Formally,
\[
\mcalr = \bigcup\limits_{k=1}^{+\infty} \mcalr_k ~\mbox{with}~
\mcalr_k = \set{\left(G ,E_*\right): G \in \mcald_k, \card{E_*} = k}
\]

%We say that, for admissible integer $D$, set $\compbin{D}$ denotes binary trees $T$ which are complete from depth 0 to depth~$D$. In other words, for any $0 \leq j \leq D$, $\card{E_T(j)} = 2^{j+1}$. The graph given in Figure~\ref{subfig:proofth1_b} belongs to $\compbin{1}$. %We also remark that if the number of edges of depth $j-1$, value $\card{E_T(j-1)}$, is $2^{j}$, then the tree is necessarily complete from depth 0 to depth $j-1$, so $\card{E_T(j-2)} = 2^{j-1}$, etc.

We pursue our reasoning by establishing properties on EBTs $T_G$ to understand the behavior of \mts es on graphs $G \in \mcalg_k$. Theorem~\ref{th:completebinary} states that cutting an edges from $G \in \mcald_k$ result on a graph in $G \in \mcald_{k-1}$.
%We pursue our reasoning by establishing properties on EBTs $T_G$ to understand the behavior of \mts es on graphs $G \in \mcalg_k$. Theorem~\ref{th:completebinary} states that for any graph $G_k\backslash E' \in \mcalg_k$, its EBT is a complete binary tree from depth 0 to $k - 1 - \card{E'}$. It is proven by induction. Figures~\ref{subfig:proofth1_a},~\ref{subfig:proofth1_b} and~\ref{subfig:proofth1_c} illustrate the induction step on an example of $\mcalg_3$.
...
\begin{theorem}
For any graph $G \in \mcald_k$, $G \backslash \set{e} \in \mcald_{k-1}$.
...
We study the competitiveness of a random \mts for every $\mcalr_k$. The Theorem~\ref{th:equalcompetitive} tell us that the competitive ratio of an optimal \mts is the same for every graph of $\mcald_k$. The proof give us a recursive formula of this competitive ratio.
...
\]
with $p_{min}$ the lowest probability. However, $p_{min} \leq \frac{1}{2^k}$ and $c_A(G, \mcalr_k)$ minimum for $p_{min}$ maximum, so $p_{min} = \frac{1}{2^k}$. The result is that $c_{\mts}(G)$ only depend on $k$ and :
\[
c_k = \cfrac{1}{2^k} + \left(1-\cfrac{1}{2^k}\right)\left(2+c_{k-1}\right)
\]
\end{proof}
