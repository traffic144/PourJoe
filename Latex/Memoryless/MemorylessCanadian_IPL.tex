...
We note $\mcalg$ the set of all these subgraphs. We call them \textit{diamond graphs} because of their appearance, diamonds joined together. Formally, $\mcalg = \bigcup_{k=1}^{+\infty} \mcalg_k ~\mbox{with}~
...
We introduce notations which characterize some elements in an \ebt . For any edge $e$ in \ebt ~$T$, we note $P(e)$ the parent edge of $e$. Let $U(e)$ denote the ``uncle'' of edge $e$, which has the same parent than $P(e)$. Put formally, $U(e) = \set{e' \in T : P(e') = P\left(P(e)\right) = P^2(e)}$. For any \ebt ~$T$, we define recursively a set of edges $E_T(l)$ which is the set of edges with depth $l$. Set $E_T(0)$ represents the edges connected to the root $t$ and $E_T(l) = \set{e : P(e) \in E_T(l-1)}$. We note $\dmin(T)$ the minimum depth among the leaves of $T$. For example, for \ebt ~$T_G$ of Figure~\ref{subfig:ebt_3}, $\dmin(T_G) = 2$. Finally, we set road atlas $\mcalr$ in Definition~\ref{def:roadatlas}.

\begin{definition}[Road atlas $\mcalr$ based on sets of graphs $\mcald_k$]
Road atlas $\mcalr$ is composed of road maps $\left(G,E_*\right)$ where $\card{E_*} = k$ and $G \in \mcalg$ with an \ebt ~fulfulling $\dmin\left(T_G\right) \geq k$. Formally, we define set $\mcald_k = \set{G \in \mcalg : \dmin(T_G) \geq k}$ and road atlas $\mcalr$ on which we study the competitiveness of \mts es,
\[
