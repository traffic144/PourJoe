...
\label{subfig:ebt_3}
\end{subfigure}
\caption{An example of graph $G \in \mcalg_2$ and its EBT $T_G$.}
\label{fig:ebt}
\end{figure}
...
For any edge $e$ in EBT $T$, we note $P(e)$ the parent edge of $e$. For any EBT $T$, we define recursively a set of edges $E_T(k)$, $E_T(0)$ being the edges connected to the root and $E_T(k) = \set{e : P(e) \in E_T(k-1)}$. In other words, $E_T(k)$ can be seen as the set of edges with depth $k$.
